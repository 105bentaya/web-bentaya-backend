...
import org.scouts105bentaya.exception.ScoutNotFoundException;
import org.scouts105bentaya.exception.user.UserNotFoundException;
import org.scouts105bentaya.repository.ContactRepository;
import org.scouts105bentaya.repository.ScoutRepository;
import org.scouts105bentaya.service.*;
...
public class ScoutServiceImpl implements ScoutService {
    ...
    public ScoutServiceImpl(ScoutRepository scoutRepository,
                            ContactRepository contactRepository, ScoutConverter scoutConverter,
                            AuthService authService, UserService userService, @Lazy EventService eventService,
                            ConfirmationService confirmationService, PreScoutService preScoutService) {
    ...
    public List<String> findScoutUsernames(Integer id) {
        ...
        return scout.getUserList().stream().map(User::getUsername).collect(Collectors.toList());
    }
    ...
    public ScoutFormUserUpdateDto getScoutFormUpdateUserMessage(Integer scoutId, List<String> newUsers) {
        ...
        if (scoutId != null) {
            ...
            result.setAddedUsers(newUsers.stream().filter(newUser -> !currentUsers.contains(newUser)).collect(Collectors.toList()));
            result.setDeletedUsers(currentUsers.stream().filter(currentUser -> !newUsers.contains(currentUser)).collect(Collectors.toList()));
        } else {
